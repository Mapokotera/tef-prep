...
function renderFeedbackPage() {
  ...
  if (!container) { console.error('[Feedback] page-feedback div not found'); return; }

  // Gather state — each in its own try/catch so nothing can crash the whole function
  let hasKey = false;
  let maskedKey = '';
  let userId = 'Loading...';
  let syncOk = false;
  ...
  try { hasKey = !!getApiKey(); if (hasKey) maskedKey = getApiKey().substring(0,14) + '••••'; } catch(e) { console.warn('[Feedback] getApiKey error:', e); }
  try { userId = getSyncUserId() || 'Click Connect to generate'; } catch(e) { console.warn('[Feedback] getSyncUserId error:', e); }
  try { syncOk = (typeof syncEnabled !== 'undefined' && syncEnabled); } catch(e) {}
  try { log = getErrorLog() || []; } catch(e) {}
  try { patterns = getErrorPatterns() || []; } catch(e) {}

  // Build page with plain string concatenation — no nested template literals
  let html = '';

  // API KEY SECTION
  if (!hasKey) {
    html += '<div class="card" style="margin-bottom:18px;border-color:rgba(79,142,247,0.3)">';
    html += '<div style="font-size:17px;font-weight:600;margin-bottom:8px">🔑 Connect API key</div>';
    html += '<div style="font-size:13px;color:var(--text2);margin-bottom:14px">Get your key at <a href="https://console.anthropic.com" target="_blank" style="color:var(--accent)">console.anthropic.com</a> — API Keys — Create Key. Costs ~$0.01 per session.</div>';
    html += '<div style="display:flex;gap:8px">';
    html += '<input type="password" id="api-key-input" placeholder="sk-ant-api-..." style="flex:1;font-family:var(--font-mono);font-size:13px">';
    html += '<button class="btn btn-primary btn-sm" onclick="handleSaveKey()">Save</button>';
    html += '</div>';
    html += '<div id="key-error" style="font-size:13px;color:var(--red);margin-top:8px;display:none"></div>';
    html += '<div style="margin-top:10px"><button class="btn btn-secondary btn-sm" style="font-size:12px" onclick="debugApiTest()">🔍 Debug test</button></div>';
    html += '<div id="debug-output" style="display:none;margin-top:10px;background:var(--bg);border:1px solid var(--border2);border-radius:8px;padding:12px;font-family:var(--font-mono);font-size:11px;color:var(--text2);white-space:pre-wrap;max-height:160px;overflow-y:auto"></div>';
    html += '</div>';
  } else {
    html += '<div style="display:flex;align-items:center;gap:10px;padding:10px 16px;background:rgba(62,207,142,0.05);border:1px solid rgba(62,207,142,0.2);border-radius:10px;margin-bottom:18px">';
    html += '<div style="width:8px;height:8px;border-radius:50%;background:var(--green)"></div>';
    html += '<div style="font-size:13px;color:var(--text2);font-family:var(--font-mono);flex:1">' + maskedKey + '</div>';
    html += '<button class="btn btn-secondary btn-sm" onclick="handleRemoveKey()" style="color:var(--red);border-color:rgba(240,96,96,0.3)">Remove</button>';
    html += '</div>';
  }

  // SYNC SECTION
  html += '<div class="card" style="margin-bottom:18px;border-color:rgba(62,207,142,0.2)">';
  html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:8px">';
  html += '<div style="font-size:17px;font-weight:600">☁️ Cross-device sync</div>';
  html += '<div style="font-size:13px;color:' + (syncOk ? 'var(--green)' : 'var(--text3)') + '">' + (syncOk ? '● Connected' : '○ Not connected') + '</div>';
  html += '</div>';
  html += '<div style="font-size:13px;color:var(--text2);margin-bottom:14px">Copy your User ID to every device to keep progress in sync.</div>';
  html += '<div style="background:var(--bg3);border-radius:10px;padding:14px;margin-bottom:12px">';
  html += '<div style="font-size:11px;color:var(--text3);font-family:var(--font-mono);margin-bottom:8px;text-transform:uppercase">Your User ID</div>';
  html += '<div style="display:flex;gap:8px">';
  html += '<input type="text" id="sync-id-input" value="' + userId + '" onclick="this.select()" style="flex:1;font-family:var(--font-mono);font-size:12px;background:var(--bg2)">';
  html += '<button class="btn btn-secondary btn-sm" onclick="copySyncId()">📋 Copy</button>';
  html += '</div>';
  html += '<div style="font-size:12px;color:var(--text3);margin-top:8px">On phone: open Feedback tab, paste this ID, tap Connect.</div>';
  html += '</div>';
  html += '<div style="display:flex;gap:8px;flex-wrap:wrap">';
  html += '<button class="btn btn-green btn-sm" onclick="handleConnectSync()">' + (syncOk ? '↺ Reconnect' : '⚡ Connect sync') + '</button>';
  if (syncOk) {
    html += '<button class="btn btn-secondary btn-sm" onclick="handleForcePush()">⬆ Push</button>';
    html += '<button class="btn btn-secondary btn-sm" onclick="handleForcePull()">⬇ Pull</button>';
    html += '<button class="btn btn-secondary btn-sm" onclick="handleDisconnectSync()" style="color:var(--red)">Disconnect</button>';
  }
  html += '</div>';
  html += '<div id="sync-message" style="display:none;margin-top:10px;padding:10px 14px;border-radius:8px;font-size:13px"></div>';
  html += '</div>';

  // FEEDBACK TABS (only if API key exists)
  if (hasKey) {
    html += '<div style="display:flex;gap:6px;margin-bottom:18px;border-bottom:1px solid var(--border);padding-bottom:14px;flex-wrap:wrap">';
    html += '<button class="fb-tab active" data-tab="writing" onclick="switchFbTab("writing")">✍️ Writing</button>';
    html += '<button class="fb-tab" data-tab="grammar" onclick="switchFbTab("grammar")">🔬 Grammar</button>';
    html += '<button class="fb-tab" data-tab="speaking" onclick="switchFbTab("speaking")">🗣️ Speaking</button>';
    html += '<button class="fb-tab" data-tab="progress" onclick="switchFbTab("progress")">📈 Progress</button>';
    html += '</div>';

    // Writing panel
    html += '<div id="fb-writing" class="fb-panel">';
    html += '<div class="card" style="margin-bottom:14px">';
    html += '<div style="font-size:16px;font-weight:600;margin-bottom:4px">Writing Feedback</div>';
    html += '<div style="font-size:13px;color:var(--text2);margin-bottom:12px">Paste your TEF task for scoring and corrections</div>';
    html += '<div style="display:flex;gap:8px;margin-bottom:10px">';
    html += '<button class="btn btn-sm" id="wfb-t1" onclick="setWFBTask(1)" style="background:rgba(79,142,247,0.15);color:var(--accent2);border:1px solid var(--accent)">Task 1 — Informal</button>';
    html += '<button class="btn btn-sm" id="wfb-t2" onclick="setWFBTask(2)" style="background:var(--bg3);border:1px solid var(--border2);color:var(--text2)">Task 2 — Formal</button>';
    html += '</div>';
    html += '<div id="wfb-target" style="font-size:12px;color:var(--text3);margin-bottom:10px">Target: 60–80 words</div>';
    html += '<textarea id="wfb-input" rows="7" placeholder="Paste your French writing here..."></textarea>';
    html += '<div style="display:flex;justify-content:space-between;margin-top:8px"><div id="wfb-wc" style="font-size:12px;color:var(--text3)">0 words</div><button class="btn btn-primary btn-sm" id="wfb-submit" onclick="submitWritingFeedback()">Get feedback →</button></div>';
    html += '</div><div id="wfb-result"></div></div>';

    // Grammar panel
    html += '<div id="fb-grammar" class="fb-panel" style="display:none">';
    html += '<div class="card" style="margin-bottom:14px">';
    html += '<div style="font-size:16px;font-weight:600;margin-bottom:4px">Grammar Checker</div>';
    html += '<div style="font-size:13px;color:var(--text2);margin-bottom:12px">Paste any French text to get every error explained</div>';
    html += '<textarea id="gfb-input" rows="7" placeholder="Collez votre texte français ici..."></textarea>';
    html += '<div style="text-align:right;margin-top:8px"><button class="btn btn-primary btn-sm" id="gfb-submit" onclick="submitGrammarCheck()">Check grammar →</button></div>';
    html += '</div><div id="gfb-result"></div></div>';

    // Speaking panel
    html += '<div id="fb-speaking" class="fb-panel" style="display:none">';
    html += '<div class="card" style="margin-bottom:14px">';
    html += '<div style="font-size:16px;font-weight:600;margin-bottom:4px">Speaking Feedback</div>';
    html += '<div style="font-size:13px;color:var(--text2);margin-bottom:12px">Type what you said — get TEF examiner-style feedback</div>';
    html += '<div style="display:flex;gap:8px;margin-bottom:10px">';
    html += '<button class="btn btn-sm" id="sfb-t1" onclick="setSFBTask(1)" style="background:rgba(155,127,232,0.15);color:var(--purple);border:1px solid var(--purple)">Task 1</button>';
    html += '<button class="btn btn-sm" id="sfb-t2" onclick="setSFBTask(2)" style="background:var(--bg3);border:1px solid var(--border2);color:var(--text2)">Task 2</button>';
    html += '<button class="btn btn-sm" id="sfb-t3" onclick="setSFBTask(3)" style="background:var(--bg3);border:1px solid var(--border2);color:var(--text2)">Task 3</button>';
    html += '</div>';
    html += '<div id="sfb-desc" style="font-size:13px;color:var(--text3);margin-bottom:10px">Task 1 — Photo description (2 minutes)</div>';
    html += '<textarea id="sfb-input" rows="7" placeholder="Type what you said during speaking practice..."></textarea>';
    html += '<div style="text-align:right;margin-top:8px"><button class="btn btn-primary btn-sm" id="sfb-submit" onclick="submitSpeakingFeedback()">Get feedback →</button></div>';
    html += '</div><div id="sfb-result"></div></div>';

    // Progress panel
    html += '<div id="fb-progress" class="fb-panel" style="display:none">';
    html += '<div class="grid-2" style="margin-bottom:14px">';
    html += '<div class="card"><div style="font-family:var(--font-display);font-size:40px;color:var(--accent)">' + log.length + '</div><div style="font-size:13px;color:var(--text2);margin-top:4px">feedback sessions</div></div>';
    html += '<div class="card"><div style="font-size:14px;font-weight:500;margin-bottom:8px">Top error patterns</div>';
    if (patterns.length) {
      patterns.slice(0,5).forEach(function(p) {
        html += '<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border);font-size:13px"><span style="color:var(--text2)">' + p[0] + '</span><span style="color:var(--amber);font-family:var(--font-mono);font-size:12px">' + p[1] + '×</span></div>';
      });
    } else {
      html += '<div style="color:var(--text3);font-size:13px">No patterns yet.</div>';
    }
    html += '</div></div>';
    if (log.length >= 3) {
      html += '<div class="card" style="margin-bottom:14px"><div style="font-size:14px;font-weight:500;margin-bottom:10px">AI Progress Analysis</div><button class="btn btn-primary btn-sm" id="prog-submit" onclick="submitProgressAnalysis()">Analyse my progress →</button><div id="prog-result" style="margin-top:14px"></div></div>';
    }
    html += '<div class="card"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px"><div style="font-size:14px;font-weight:500">Recent sessions</div><button class="btn btn-secondary btn-sm" onclick="clearErrorLog()" style="color:var(--red);font-size:12px">Clear</button></div>';
    if (log.length) {
      [...log].reverse().slice(0,8).forEach(function(e) {
        html += '<div style="padding:8px 0;border-bottom:1px solid var(--border);font-size:13px">';
        html += '<div style="display:flex;justify-content:space-between"><span style="color:var(--text2)">' + (e.type||'Session') + '</span><span style="color:var(--text3);font-family:var(--font-mono);font-size:12px">' + new Date(e.date).toLocaleDateString() + '</span></div>';
        if (e.patterns && e.patterns.length) html += '<div style="color:var(--text3);font-size:12px;margin-top:2px">' + e.patterns.join(' · ') + '</div>';
        html += '</div>';
      });
    } else {
      html += '<div style="color:var(--text3);font-size:13px">No sessions yet.</div>';
    }
    html += '</div></div>';
  } else {
    html += '<div class="card" style="text-align:center;padding:28px;color:var(--text2)">Add your API key above to unlock writing, grammar and speaking feedback.</div>';
  }

  container.innerHTML = html;

  // Wire word count for writing tab
  const wfbInput = document.getElementById('wfb-input');
  if (wfbInput) {
    wfbInput.addEventListener('input', function() {
      const wc = document.getElementById('wfb-wc');
      if (wc) wc.textContent = this.value.trim().split(/\s+/).filter(Boolean).length + ' words';
    });
  }
}
      ...
      <div style="display:flex;gap:16px;align-items:flex-start">
        ...
        <div style="flex:1">
          ...
          <div style="display:flex;gap:8px">
            <input type="password" id="api-key-input" placeholder="sk-ant-api-..." style="flex:1;font-family:var(--font-mono);font-size:13px">
          ...
          <div style="margin-top:10px">
            <button class="btn btn-secondary btn-sm" onclick="debugApiTest()" style="font-size:12px">🔍 Debug — show raw API response</button>
          </div>
          <div id="debug-output" style="display:none;margin-top:10px;background:var(--bg);border:1px solid var(--border2);border-radius:8px;padding:12px;font-family:var(--font-mono);font-size:12px;color:var(--text2);white-space:pre-wrap;word-break:break-all;max-height:200px;overflow-y:auto"></div>
        </div>
      </div>
    </div>
  `;
...
async function handleSaveKey() {
  ...
  if (!key || !key.startsWith('sk-')) {
    errEl.textContent = 'Key should start with sk-ant-api-... — check you copied the full key.';
  ...
  try {
    // Validate by hitting the API directly — bypass callClaude to get raw status
    ...
    if (!testRes.ok) {
      // Show the exact error Anthropic returns
      const apiMsg = body?.error?.message || `HTTP ${testRes.status}`;
...
async function debugApiTest() {
  ...
  if (!key) {
    output.textContent = 'No key entered. Paste your key in the field above first.';
    return;
  }

  output.textContent = `Sending test request with key: ${key.substring(0,12)}...\n`;
  ...
  try {
    ...
    output.textContent += `Response headers: ${JSON.stringify(headers, null, 2)}\n`;

...
async function handleSaveKey() {
  ...
  try {
    ...
    if (errEl) {
      errEl.textContent = e.message === 'INVALID_KEY' ? 'Invalid API key — please check and try again.' : 'Could not validate key. Check your internet connection.';
...
async function submitWritingFeedback() {
  ...
  try {
    ...
    if (e.message === 'NO_KEY' || e.message === 'INVALID_KEY') {
      showError('wfb-result', 'API key issue. Go to the top of this page to re-enter your key.');
    } else {
      showError('wfb-result', `Something went wrong: ${e.message}`);
    }
  } finally {
...
async function submitGrammarCheck() {
  ...
  try {
    ...
    if (e.message === 'NO_KEY' || e.message === 'INVALID_KEY') {
      showError('gfb-result', 'API key issue. Go to the top of this page to re-enter your key.');
    } else {
      showError('gfb-result', `Something went wrong: ${e.message}`);
    }
  } finally {
...
async function submitSpeakingFeedback() {
  ...
  try {
    ...
    if (e.message === 'NO_KEY' || e.message === 'INVALID_KEY') {
      showError('sfb-result', 'API key issue. Go to the top of this page to re-enter your key.');
    } else {
      showError('sfb-result', `Something went wrong: ${e.message}`);
    }
  } finally {
    btn.disabled = false; btn.textContent = 'Get feedback →';
  }

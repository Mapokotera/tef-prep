...
function renderFeedbackPage() {
  ...
  try { hasKey = !!getApiKey(); } catch(e) {}
  try { userId = getSyncUserId() || ''; } catch(e) {}
  try { syncConnected = typeof syncEnabled !== 'undefined' && syncEnabled && !!syncUserId; } catch(e) {}
  try { patterns = getErrorPatterns(); } catch(e) {}
  try { log = getErrorLog(); } catch(e) {}

  const maskedKey = hasKey ? (getApiKey().substring(0,12) + '••••••••••••') : '';

  container.innerHTML = `
    <!-- ── API KEY ── -->
    <div style="margin-bottom:20px">
      ${!hasKey ? `
        <div class="card" style="border-color:rgba(79,142,247,0.3);background:rgba(79,142,247,0.03)">
          <div style="display:flex;gap:14px;align-items:flex-start">
            ...
            <div style="flex:1">
              ...
              <div style="font-size:13px;color:var(--text2);margin-bottom:14px;line-height:1.6">
                ...
                Cost: ~$0.01–0.03 per feedback session.
              </div>
              <div style="display:flex;gap:8px">
                <input type="password" id="api-key-input" placeholder="sk-ant-api-..." style="flex:1;font-family:var(--font-mono);font-size:13px">
                <button class="btn btn-primary btn-sm" onclick="handleSaveKey()">Save key</button>
              </div>
              <div id="key-error" style="font-size:13px;color:var(--red);margin-top:8px;display:none"></div>
              <div style="margin-top:10px">
                <button class="btn btn-secondary btn-sm" onclick="debugApiTest()" style="font-size:12px">🔍 Debug — show raw response</button>
              </div>
              <div id="debug-output" style="display:none;margin-top:10px;background:var(--bg);border:1px solid var(--border2);border-radius:8px;padding:12px;font-family:var(--font-mono);font-size:11px;color:var(--text2);white-space:pre-wrap;word-break:break-all;max-height:180px;overflow-y:auto"></div>
            </div>
          </div>
        </div>
      ` : `
        <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;background:rgba(62,207,142,0.05);border:1px solid rgba(62,207,142,0.2);border-radius:10px">
          <div style="width:8px;height:8px;border-radius:50%;background:var(--green);flex-shrink:0"></div>
          <div style="font-size:13px;color:var(--text2);font-family:var(--font-mono);flex:1">${maskedKey}</div>
          <button class="btn btn-secondary btn-sm" onclick="handleRemoveKey()" style="color:var(--red);border-color:rgba(240,96,96,0.3)">Remove</button>
        </div>
      `}
    </div>
    ...
    <div class="card" style="margin-bottom:20px;border-color:rgba(62,207,142,0.2);background:rgba(62,207,142,0.02)">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px">
        <div class="card-title" style="font-size:17px">☁️ Cross-device sync</div>
        <div style="font-size:13px;color:${syncConnected?'var(--green)':'var(--text3)'}">${syncConnected?'● Connected':'○ Not connected'}</div>
      </div>
      <div style="font-size:13px;color:var(--text2);margin-bottom:14px;line-height:1.6">
        Use the <strong style="color:var(--text)">same User ID</strong> on every device to keep your progress in sync.
      </div>
      <div style="background:var(--bg3);border-radius:10px;padding:14px;margin-bottom:12px">
        <div style="font-size:11px;color:var(--text3);font-family:var(--font-mono);margin-bottom:8px;text-transform:uppercase;letter-spacing:.06em">Your User ID</div>
        <div style="display:flex;gap:8px">
          <input type="text" id="sync-id-input" value="${userId}" onclick="this.select()"
            style="flex:1;font-family:var(--font-mono);font-size:13px;letter-spacing:0.04em;background:var(--bg2)"
            placeholder="Generating...">
          <button class="btn btn-secondary btn-sm" onclick="copySyncId()" style="white-space:nowrap">📋 Copy</button>
        </div>
        <div style="font-size:12px;color:var(--text3);margin-top:8px">On your phone: open ⚡ Feedback, paste this ID above, tap <strong style="color:var(--text2)">Connect sync</strong>.</div>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <button class="btn btn-green btn-sm" onclick="handleConnectSync()">${syncConnected?'↺ Reconnect':'⚡ Connect sync'}</button>
        ${syncConnected?'<button class="btn btn-secondary btn-sm" onclick="handleForcePush()">⬆ Push</button><button class="btn btn-secondary btn-sm" onclick="handleForcePull()">⬇ Pull</button><button class="btn btn-secondary btn-sm" onclick="handleDisconnectSync()" style="color:var(--red);border-color:rgba(240,96,96,.3)">Disconnect</button>':''}
      </div>
    ...
    ${hasKey ? `
    <!-- ── TABS ── -->
    <div style="display:flex;gap:6px;margin-bottom:20px;border-bottom:1px solid var(--border);padding-bottom:14px;flex-wrap:wrap">
      <button class="fb-tab active" data-tab="writing" onclick="switchFbTab('writing')">✍️ Writing</button>
      <button class="fb-tab" data-tab="grammar" onclick="switchFbTab('grammar')">🔬 Grammar</button>
    ...
    <div id="fb-writing" class="fb-panel">
      <div class="card" style="margin-bottom:14px">
        <div class="card-title" style="font-size:17px;margin-bottom:4px">Writing Feedback</div>
        <div style="font-size:13px;color:var(--text2);margin-bottom:14px">Paste your TEF task for scoring and corrections</div>
        ...
        <div id="wfb-target" style="font-size:12px;color:var(--text3);margin-bottom:10px;font-family:var(--font-mono)">Target: 60–80 words</div>
        <textarea id="wfb-input" rows="7" placeholder="Paste your French writing here..." oninput="document.getElementById('wfb-wc').textContent=this.value.trim().split(/\s+/).filter(Boolean).length+' words'"></textarea>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:7px">
          <div id="wfb-wc" style="font-size:12px;color:var(--text3);font-family:var(--font-mono)">0 words</div>
          <button class="btn btn-primary btn-sm" onclick="submitWritingFeedback()" id="wfb-submit">Get feedback →</button>
        </div>
      </div>
    ...
    <div id="fb-grammar" class="fb-panel" style="display:none">
      <div class="card" style="margin-bottom:14px">
        <div class="card-title" style="font-size:17px;margin-bottom:4px">Grammar Checker</div>
        <div style="font-size:13px;color:var(--text2);margin-bottom:14px">Paste any French text to get every error explained</div>
        <textarea id="gfb-input" rows="7" placeholder="Collez votre texte français ici..."></textarea>
        <div style="text-align:right;margin-top:8px">
          <button class="btn btn-primary btn-sm" onclick="submitGrammarCheck()" id="gfb-submit">Check grammar →</button>
        </div>
      </div>
    ...
    <div id="fb-speaking" class="fb-panel" style="display:none">
      <div class="card" style="margin-bottom:14px">
        <div class="card-title" style="font-size:17px;margin-bottom:4px">Speaking Feedback</div>
        <div style="font-size:13px;color:var(--text2);margin-bottom:14px">Type what you said — get TEF examiner-style feedback</div>
        <div style="display:flex;gap:8px;margin-bottom:10px">
        ...
        <div id="sfb-desc" style="font-size:13px;color:var(--text3);margin-bottom:10px">Task 1 — Photo description (2 minutes)</div>
        <textarea id="sfb-input" rows="7" placeholder="Type what you said during speaking practice..."></textarea>
        <div style="text-align:right;margin-top:8px">
          <button class="btn btn-primary btn-sm" onclick="submitSpeakingFeedback()" id="sfb-submit">Get feedback →</button>
        </div>
      </div>
    ...
    <div id="fb-progress" class="fb-panel" style="display:none">
      <div class="grid-2" style="margin-bottom:16px">
        <div class="card">
          <div style="font-family:var(--font-display);font-size:42px;color:var(--accent)">${log.length}</div>
          <div style="font-size:13px;color:var(--text2);margin-top:4px">feedback sessions logged</div>
        </div>
        <div class="card">
          <div style="font-size:13px;font-weight:500;color:var(--text);margin-bottom:10px">Top error patterns</div>
          ${patterns.length ? patterns.slice(0,5).map(([p,c])=>'<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border);font-size:13px"><span style="color:var(--text2)">'+p+'</span><span style="font-family:var(--font-mono);font-size:12px;color:var(--amber)">'+c+'×</span></div>').join('') : '<div style="color:var(--text3);font-size:13px">No patterns yet — submit some feedback sessions first.</div>'}
        </div>
      </div>
      ${log.length>=3?'<div class="card" style="margin-bottom:14px"><div style="font-size:15px;font-weight:500;margin-bottom:10px">AI Progress Analysis</div><button class="btn btn-primary btn-sm" onclick="submitProgressAnalysis()" id="prog-submit">Analyse my progress →</button><div id="prog-result" style="margin-top:14px"></div></div>':'<div class="card" style="margin-bottom:14px;text-align:center;color:var(--text3);font-size:14px;padding:24px">Complete 3+ feedback sessions to unlock AI progress analysis.</div>'}
      <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <div style="font-size:15px;font-weight:500">Recent sessions</div>
          <button class="btn btn-secondary btn-sm" onclick="clearErrorLog()" style="color:var(--red);border-color:rgba(240,96,96,.3);font-size:12px">Clear</button>
        </div>
        ${log.length?[...log].reverse().slice(0,8).map(e=>'<div style="padding:9px 0;border-bottom:1px solid var(--border);font-size:13px"><div style="display:flex;justify-content:space-between"><span style="color:var(--text2);font-weight:500">'+(e.type||'Session')+'</span><span style="color:var(--text3);font-family:var(--font-mono)">'+new Date(e.date).toLocaleDateString()+'</span></div>'+(e.patterns?.length?'<div style="color:var(--text3);margin-top:2px;font-size:12px">'+e.patterns.join(' · ')+'</div>':'')+'</div>').join(''):'<div style="color:var(--text3);font-size:13px">No sessions yet.</div>'}
      </div>
    </div>
    ` : '<div class="card" style="text-align:center;padding:32px;color:var(--text2)">Add your API key above to unlock writing, grammar and speaking feedback.</div>'}
  `;
}
